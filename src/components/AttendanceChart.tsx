    ...
    return (
        <div style={{ width: "100%", height: 360 }}>
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 12, left: -10, bottom: 6 }}>
                    <CartesianGrid strokeDasharray="4 4" stroke="#eef2f6" />
                    ...
                    <Legend />
                    {/* Present (blue) */}
                    <Bar dataKey="present" name="Hadir" fill="#0B4DA3" radius={[6, 6, 0, 0]} barSize={18}>
                        <LabelList dataKey="present" position="top" formatter={(val: React.ReactNode) => String(val)} />
                    </Bar>
                    {/* Absent (danger) */}
                    <Bar dataKey="absent" name="Tidak Hadir" fill="#ef4444" radius={[6, 6, 0, 0]} barSize={18}>
